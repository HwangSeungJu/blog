...
public class TeamDAO_old {
	...
	public boolean create(TeamDTO dto){
		...
		sql.append(" (no,name,gender,phone,zipcode,address1, ");
		sql.append("  address2,skill,hobby,filename) ");
		sql.append(" values ");
	...
	public List<TeamDTO> list(Map map){
		...
		sql.append(" 	rownum r ");
		sql.append(" 	from( ");
		sql.append(" 		select no, name, phone, skill,  ");
		sql.append(" 		filename from team ");
		if(word.trim().length()>0)
			sql.append(" 	where "+col+" like '%'||?||'%' ");
