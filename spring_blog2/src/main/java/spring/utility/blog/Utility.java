...
public class Utility {
	 ...
	 private static final String root ="/calendar";
...
//		    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
	...
	public static void deleteFile(String upDir, String oldfile) {
	  ...
	  * @return 페이징 생성 문자열
	  */ 
	 public static String memopaging(int totalRecord, int nPage, int recordPerPage,String url,int memono,int nowPage, String col, String word){ 
	   int pagePerBlock = 10; // 블럭당 페이지 수 
	   int totalPage = (int)(Math.ceil((double)totalRecord/recordPerPage)); // 전체 페이지  
	   int totalGrp = (int)(Math.ceil((double)totalPage/pagePerBlock));// 전체 그룹 
...
//	     str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
	   ...
	   int _nowPage = (nowGrp-1) * pagePerBlock; // 10개 이전 페이지로 이동 
	   if (nowGrp >= 2){ 
	     str.append("<span class='span_box_1'><A href='./"+url+"?nowPage="+nowPage+"&col="+col+"&word="+word+"&memono="+memono+"&nPage="+_nowPage+"'>이전</A></span>"); 
	   } 
	   ...
	   for(int i=startPage; i<=endPage; i++){ 
	     ...
	     if (nPage == i){ 
	       ...
	       str.append("<span class='span_box_1'><A href='./"+url+"?nowPage="+nowPage+"&col="+col+"&word="+word+"&memono="+memono+"&nPage="+i+"'>"+i+"</A></span>");   
	     } 
	   } 
	    
	   _nowPage = (nowGrp * pagePerBlock)+1; // 10개 다음 페이지로 이동 
	   if (nowGrp < totalGrp){ 
	     str.append("<span class='span_box_1'><A href='./"+url+"?nowPage="+nowPage+"&col="+col+"&word="+word+"&memono="+memono+"&nPage="+_nowPage+"'>다음</A></span>"); 
	   } 
	   str.append("</DIV>"); 
